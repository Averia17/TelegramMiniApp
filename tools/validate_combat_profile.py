...
def _require_keys(errors: list[str], value: Any, path: str, keys: tuple[str, ...]) -> None:
...
def _reject_unknown_keys(errors: list[str], value: Any, path: str, keys: tuple[str, ...]) -> None:
    if not isinstance(value, dict):
        return
    allowed = set(keys)
    for key in value:
...
def validate_profile(
    ...
    _require_keys(
        ...
        profile,
        "profile",
        ("schemaVersion", "profileId", "profileRevision", "modes", "defaults", "heroes"),
    )
    _reject_unknown_keys(
        errors,
        profile,
        "profile",
        ("$schema", "schemaVersion", "profileId", "profileRevision", "purpose", "modes", "defaults", "heroes"),
    )
    ...
    _require_keys(
        ...
        defaults,
        "profile.defaults",
        ("super", "gadget", "healthBoost", "respawn", "pickup", "loot", "bats", "telegraph", "ai"),
    )
    if isinstance(defaults, dict):
        ...
        _require_keys(errors, defaults.get("bats"), "profile.defaults.bats", ("role", "respawnPolicy", "target"))
        _require_keys(errors, defaults.get("telegraph"), "profile.defaults.telegraph", ("minDurationMs",))
        ...
        if isinstance(loot, dict):
            ...
            if loot.get("healthPickupIds") != ["health_boost"]:
                errors.append("profile.defaults.loot.healthPickupIds must contain only health_boost")
            health_ids = set(loot.get("healthPickupIds", [])) if isinstance(loot.get("healthPickupIds"), list) else set()
            non_hp_ids = set(loot.get("nonHpPickupIds", [])) if isinstance(loot.get("nonHpPickupIds"), list) else set()
            if health_ids & non_hp_ids:
                errors.append("profile.defaults.loot health and non-HP pickup ids must not overlap")
            if loot.get("nonHpPickupMode") != "optional_bonus":
                errors.append("profile.defaults.loot.nonHpPickupMode must be optional_bonus")
            if loot.get("nonHpAffectsCurrentLives") is not False:
                errors.append("profile.defaults.loot.nonHpAffectsCurrentLives must be false")

    catalog_heroes = catalog.get("heroes", [])
    active_catalog = {
        hero.get("id"): hero for hero in catalog_heroes if hero.get("status") == "active"
    }
    ...
    for hero_id, contract in heroes.items():
        ...
        if not isinstance(contract, dict):
            errors.append(f"{path} must be an object")
            continue
        _reject_unknown_keys(errors, contract, "profile.heroes.%s" % hero_id, ("role", "powerBudget", "powerBudgetVector", "basic", "super", "gadget"))
        ...
        _require_number(errors, contract.get("powerBudget"), f"{path}.powerBudget", minimum=0.1)
        budget = contract.get("powerBudgetVector")
        _require_keys(errors, budget, f"{path}.powerBudgetVector", budget_keys)
        if isinstance(budget, dict):
            for key in budget_keys:
                _require_fraction(errors, budget.get(key), f"{path}.powerBudgetVector.{key}")
            if not any(_is_number(budget.get(key)) and budget[key] >= 0.85 for key in budget_keys):
                errors.append(f"{path}.powerBudgetVector must have a signature value >= 0.85")
        ...
        if isinstance(basic, dict):
            ...
            comparisons = {
                "abilityId": (catalog_hero.get("abilities", {}).get("basic", {}).get("id"), "ability id"),
                "attackType": (catalog_balance.get("attackType"), "attack type"),
                "attackDamage": (catalog_balance.get("attackDamage"), "attack damage"),
                "attackCooldownMs": (catalog_balance.get("attackRateMs"), "attack cooldown"),
        ...
        for slot in ("super", "gadget"):
            ...
            if isinstance(ability, dict):
                allowed = ("abilityId", "chargeWeight") if slot == "super" else ("abilityId", "maxCharges")
        ...
        if isinstance(contract.get("super"), dict):
            _require_number(errors, contract["super"].get("chargeWeight"), f"{path}.super.chargeWeight", minimum=0.1)
        if isinstance(contract.get("gadget"), dict):
            _require_number(errors, contract["gadget"].get("maxCharges"), f"{path}.gadget.maxCharges", minimum=1)
