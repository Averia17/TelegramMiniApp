...
const HARNESS = process.env.HARNESS_URL || "http://127.0.0.1:4173/test/glb-hero-harness.html"
const OUT = path.join(ROOT, "artifacts", "needle-browser-qa")
const cases = [
...
(async () => {
  ...
  for (const [name, selector, expectedClip, predicate] of cases) {
    ...
    await page.waitForTimeout(name === "spawn" ? 120 : 180)
    const state = JSON.parse(await page.evaluate(() => window.render_game_to_text()))
    const valid = clips.includes(expectedClip) && predicate(state) && state.fallbackEvents.length === 0
    results.push({name, expectedClip, valid, animation: state.animation, overlay: state.overlay, actionWeights: state.actionWeights})
    if (name === "attack" || name === "aim-gadget") await page.screenshot({path: path.join(OUT, `${name}.png`), fullPage: true})
  }
  const report = {hero: "needle", clips, cases: results, consoleErrors, pageErrors, status: results.every(item => item.valid) && !consoleErrors.length && !pageErrors.length ? "PASS" : "FAIL"}
