...
export class LocalBattleEngine {
  ...
  shoot = angle => {
    ...
    this.performAttack("demo-player", worldAngleFromScreen(angle), performance.now())
  }

  performAttack(owner, angle, now) {
    ...
    else if(hero==="frost"){source.heat=Math.min(5,(source.heat||0)+1);source.heatTime=2.2;[-.035,-.02,-.008,.008,.02,.035].forEach(o=>this.spawnBullet(owner,angle+o,source.heat>=5?"#ff8b3d":"#54f2ff",180,{kind:source.heat>=5?"firebeam":"laser",speed:690,acceleration:420,size:4,life:.95,pierce:source.heat>=5?2:0,ignite:source.heat>=5}))}
    else if(hero==="viper")this.meleeAttack(owner,angle,145,.72,1250,"slam","#ff7138",target=>{target.slow=1.2})
    else if(hero==="titan"){const disc=this.state.bullets.find(b=>b.playerId===owner&&b.kind==="boomerang"&&b.life>0);if(disc){source.x=disc.x;source.y=disc.y;disc.life=0;this.radialAttack(owner,105,600,"rewind","#58f6e9");this.impact(10,.025)}else this.spawnBullet(owner,angle,"#58f6e9",650,{kind:"boomerang",speed:520,acceleration:260,size:11,life:1.65,pierce:1})}
    else if(hero==="shadow")this.spawnBullet(owner,angle,"#75d947",750,{kind:"spore",speed:450,size:15,life:1.15,splash:115})
    ...
    else if(hero==="rex"){this.meleeAttack(owner,angle,92,.82,1200,"slash","#4bc7ff");source.shield=(source.shield||0)+1.2}
    else if(hero==="pixel")this.spawnBullet(owner,angle,"#ffdf4b",850,{kind:"quantum",speed:580,size:10,life:1.2,splash:75,pierce:0})
  ...
  hitTarget(item, damage, owner) {
    ...
    const applied = item.monster || target.shield <= 0 ? damage : Math.round(damage * .6)
    ...
    if (attacker && owner !== item.id) attacker.superCharge = Math.min(100, (attacker.superCharge || 0) + 20)
    if (!target.lives) {
      if (item.monster) this.defeatMonster(item.id, target, owner)
        ...
        if(attackerHero==="blaze"&&(attacker?.dashResetWindow||0)>0)this.cooldowns.secondary=0
      }
    }
  }
  ...
  meleeAttack(owner, angle, range, arc, damage, kind, color, onHit) {
    ...
    this.enemyTargets(owner).forEach(item => {
      ...
      if (Math.hypot(dx, dy) <= range && Math.abs(delta) <= arc) { this.hitTarget(item, damage, owner); onHit?.(item.target) }
    })
  ...
  chainPrimary(owner, range, count) {
    ...
    let from = source
    hits.forEach(item => { this.hitTarget(item, 650); this.addEffect({kind: "lightning", x: from.x, y: from.y, toX: item.target.x, toY: item.target.y, color: "#65efff", life: .26}); from = item.target })
  }
  ...
  ability = slot => {
    ...
    if(slot==="primary"){
      ...
      else if(hero==="rex"){this.tryMove(player,Math.cos(player.rotation)*230,Math.sin(player.rotation)*230);this.areaDamage(player,145,1400);this.enemyTargets("demo-player").forEach(({target})=>{if(Math.hypot(target.x-player.x,target.y-player.y)<145)target.stun=.7});this.destroyTilesInRadius(player.x,player.y,175);this.cooldowns.primary=5.2}
      else if(hero==="pixel"){this.tryMove(player,Math.cos(player.rotation)*100,Math.sin(player.rotation)*100);player.invulnerable=.35;player.evolution=Math.min(4,(player.evolution||0)+1);player.ammo=1;this.radialAttack("demo-player",100,650,"thruster","#65efff");this.cooldowns.primary=5.2}
      ...
      if(hero==="blaze"){this.tryMove(player,Math.cos(player.rotation)*175,Math.sin(player.rotation)*175);player.dashResetWindow=3;this.addEffect({kind:"smoke",x:player.x,y:player.y,radius:80,color:"#d38cff",life:1.2})}
      ...
      if(hero==="nova"){this.tryMove(player,Math.cos(player.rotation)*240,Math.sin(player.rotation)*240);player.ammo=3}
  ...
  destroyTilesInRadius(worldX, worldY, radius) {
  ...
  tick = now => {
    ...
    Object.values(this.state.players).forEach(target => {
      ["stun","grounded","invulnerable","dashResetWindow","channel","stealth","vortex","vine","airborne","bulletTime","flying","heatTime"].forEach(k=>target[k]=Math.max(0,(target[k]||0)-dt))
      if(target.heatTime<=0)target.heat=Math.max(0,(target.heat||0)-dt*2)
      if((target.poisonTime||0)>0&&!target.dead){target.poisonTime=Math.max(0,target.poisonTime-dt);target.poisonTick=(target.poisonTick||0)-dt;if(target.poisonTick<=0){target.poisonTick=.5;target.lives=Math.max(0,target.lives-(target.poisonDamage||150));if(!target.lives)target.dead=true}}
    })
    ...
    if (local) {
      ...
      if(local.flying>0){this.addEffect({kind:"acid",x:local.x,y:local.y,radius:75,color:"#5f2a72",life:1.6});this.enemyTargets("demo-player").forEach(({target})=>{if(Math.hypot(target.x-local.x,target.y-local.y)<110)target.blind=1})}
    }
  ...
  updatePlayer(dt) {
    ...
    const speedBoost = (player.haste > 0 ? 1.55 : 1) * ((player.heat||0)>=5?1.4:1) * (player.bulletTime>0?1.4:1)
  ...
  tryMove(entity, dx, dy) {
    ...
    for(let step=0;step<steps;step+=1){
      ...
      const xBlocked=this.state.map.walls.some(wall=>blocksMovement(wall)&&circleHitsRect(nextX,entity.y,radius,wall))
      const yBlocked=this.state.map.walls.some(wall=>blocksMovement(wall)&&circleHitsRect(entity.x,nextY,radius,wall))
  ...
  updateBullets(dt) {
    this.state.bullets.forEach(bullet => {
      ...
      Object.entries(this.state.players).forEach(([id, target]) => {
        ...
        if (segmentHitsCircle(previousX, previousY, bullet.x, bullet.y, target.x, target.y, 24 + (bullet.size || 7))) {
          ...
          const appliedDamage = target.shield > 0 ? Math.round(rawDamage*.6) : rawDamage
  ...
  updatePickups() {
    ...
    this.state.props = this.state.props.filter(prop => {
      ...
      } else player.lives = Math.min(player.maxLives, player.lives + 800)
      return false
    })
  }
