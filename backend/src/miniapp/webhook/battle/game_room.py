...
class GameRoom:
    ...
    def set_simulation_interval(self, callback, delay=1000 / 60):
        self.simulation_interval = {"callback": callback, "delay": delay, "last_time": datetime.now().timestamp()}

    def broadcast(self, message_type: str, message: Dict[str, Any]):
        for client in self.clients.values():
            asyncio.create_task(client.send(message_type, message))

    def on_create(self, options: Dict[str, Any]):
        self.max_clients = self._clamp(
            options.get("roomMaxPlayers", 0), Constants.ROOM_PLAYERS_MIN, Constants.ROOM_PLAYERS_MAX
        )
        ...
        print(
            f"{datetime.now().isoformat()} [Create] player={player_name} room={room_name} "
