...
def upgrade() -> None:
    op.create_table("playerwallets",
        ...
        sa.Column("energy_updated_at", postgresql.TIMESTAMP(timezone=True), server_default=sa.text("now()"), nullable=False),
        sa.PrimaryKeyConstraint("user_id"),
        sa.CheckConstraint("gold >= 0", name="ck_wallet_gold_nonnegative"),
        sa.CheckConstraint("energy >= 0 AND energy <= 100", name="ck_wallet_energy_range"),
    )
    op.create_table("processedbattles",
