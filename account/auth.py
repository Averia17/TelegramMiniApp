...
def verify_telegram_init_data(init_data: str, max_age_seconds: int = 3600) -> int:
    ...
    if not received_hash or auth_date <= 0 or abs(time.time() - auth_date) > max_age_seconds:
    ...
    try:
        ...
        raise HTTPException(status_code=401, detail="Telegram user is missing") from None
    if user_id <= 0:
